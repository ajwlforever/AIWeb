package com.yqx.ai.controller;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.google.gson.Gson;
import com.yqx.ai.common.*;
import com.yqx.ai.config.dto.chart.*;
import com.yqx.ai.model.entity.User;
import com.yqx.ai.model.utils.SqlUtils;
import com.yqx.ai.annotation.AuthCheck;

import com.yqx.ai.constant.CommonConstant;
import com.yqx.ai.constant.UserConstant;
import com.yqx.ai.exception.BusinessException;
import com.yqx.ai.exception.ThrowUtils;
import com.yqx.ai.manager.AiManager;
import com.yqx.ai.model.entity.Chart;
import com.yqx.ai.model.vo.BiReseponse;
import com.yqx.ai.service.ChartService;
import com.yqx.ai.service.UserService;
...
public class ChartController {
    ...
    public void file(@PathVariable("fileName")String fileName, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR,"文件没找到");
        } catch (IOException e) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR,"文件传输出问题");
        }
    ...
    public BaseResponse<BiReseponse> genChartByAi(@RequestPart("file") MultipartFile multipartFile,
        ...
        genParamsCheck(goal, name, multipartFile);
        String csvData = ExcelUtils.excelToCSV(multipartFile);

        // multifile存在服务器上
        String fileName = CommonUtil.generateUUID().toString() + "-" + user.getId()+"-"+ multipartFile.getOriginalFilename();
        String filePath = uploadPath + "\\" + fileName;
        File file = new File(filePath);
        ...
        try {
            ...
            throw  new BusinessException(ErrorCode.SYSTEM_ERROR, "文件上传失败，请重试！");
        }
        String dataUrl = domain+":"+port+"/"+contextPath+"/chart/file/"+fileName;
       // System.out.println(dataUrl);

        String ans = aiManager.doDataAnalysis(goal, csvData);
        ...
        chart.setChartType("");
        chart.setGenChart("");
        chart.setGetResult(ans);
        chart.setUserId(user.getId());
        chartService.save(chart);

        BiReseponse biReseponse = new BiReseponse();

        biReseponse.setGenResult(ans);

        return  ResultUtils.success(biReseponse);

    }
    public void genParamsCheck(String name, String goal, MultipartFile multipartFile){
        // 生成数据分析结果，参数检查
        ThrowUtils.throwIf(StringUtils.isEmpty(goal) || StringUtils.isEmpty(name) ,
                ErrorCode.PARAMS_ERROR, "参数错误:有参数为空");
        // 字符过长
        ThrowUtils.throwIf(goal.length() > 300 || name.length() > 100,
                ErrorCode.PARAMS_ERROR, "参数错误:有参数过长");


        //  判断文件是否合法
        ThrowUtils.throwIf(multipartFile == null || multipartFile.isEmpty(),
                ErrorCode.PARAMS_ERROR, "参数错误:文件为空");
        //  判断文件大小
        ThrowUtils.throwIf(multipartFile.getSize() > 1024 * 1024 * 10,
                ErrorCode.PARAMS_ERROR, "参数错误:文件大小超过10M");
        //  判断文件后缀
        ThrowUtils.throwIf(!multipartFile.getOriginalFilename().endsWith(".xlsx"),
                ErrorCode.PARAMS_ERROR, "参数错误:文件后缀不正确");
    }
